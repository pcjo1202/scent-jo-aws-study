...
- **`exit 0`은 "통과"가 아니라 "오류 없음"이다.** 0건 실행과 0건 실패는 똑같이 0을 준다. 검증은 **실행 건수와 출력 전문**을 함께 본다 — 출력을 `tail`로 자르지 않고, 린터·테스트 probe는 레포 안에서 돌린다. 대상이 0건이면 통과가 아니라 **"대상 없음"으로 보고**한다. **부분 집계도 같다** — 1019 중 999처럼 그럴듯하게 조금 모자란 숫자가 0건보다 위험하다. 사실로 보고하기 전에 그 숫자를 만든 도구가 대상 전부에 닿았는지 본다 (애드혹 grep 말고 파이프라인이 쓰는 모듈로).
  **산술이 만든 수도 같다 — 두 집합의 크기를 빼기 전에 포함 관계를 잰다.** `|A| - |B|`가 뜻을 가지려면 `B ⊆ A`여야 하는데 그건 가정이 아니라 측정값이다. 세 수(`|A∩B|` · `|A\B|` · `|B\A|`)를 전부 내면 포함 관계가 드러나고 어느 쪽이 답인지도 정해진다. 코드는 뺄셈이 아니라 **교집합 판정**으로 쓴다 — 그래야 수가 틀려도 화면은 안 틀린다. 그리고 **낡은 문장을 근거로 인용할 때가 가장 위험하다**: 인용은 그 문장을 참으로 만들지 않는데 인용하는 순간 검증한 기분이 든다 (2026-09-09 SJO-25 — 「202 중 136이니 나머지 66」이 이틀간 정본이었고, 반례가 되는 「7개」가 같은 레포에 이미 적혀 있었는데 한 번도 대조되지 않았다).
...
- **규칙을 고치면 그 규칙을 참조하는 곳을 전수로 훑는다.** 산문만 고치고 요약을 남기면 **표와 본문이 반대를 말하는데 둘 다 정본으로 읽혀** 공백보다 위험하다. 남는 자리는 정해져 있다 — ① 요약 **표**의 셀 ② 하드코딩된 **상수**(「일곱 개」·「64px」) ③ **절대 표현**(「~뿐이다」·「항상」·「전부」·「하나다」) ④ `「」`로 인용한 **소제목** ⑤ "~이므로"로 폐기된 전제를 근거로 든 문장. 애드혹으로 눈에 띄는 것만 고치면 매번 남는다 — 바뀐 **값·역할명·소제목 전문**을 검색어로 만들어 기계적으로 뽑고 **개수를 센다**. ⑤가 가장 나쁘다: 근거가 거짓인 채 결론만 맞는 문장은 다음 사람이 그대로 인용한다.
  **검색어는 내가 쓴 표기로만 만들지 않는다** — 한국어는 어순·조사·띄어쓰기가 갈려 「화면 8개」와 「8개 화면」이 문자열로 안 겹친다. 변형을 다 못 세겠으면 **값만으로 훑고**(`8`) 눈으로 거른다. **범위에 코드를 넣는다** — 같은 상수가 주석·`aria-label`·화면 문구로 복제돼 있고, 「문서 작업」이라는 프레임이 범위를 문서로 좁힌다. 그리고 **내 diff 자신도 대상이다**: 파급을 훑는 그 변경이 새 절대 표현을 깨뜨린다 (2026-09-08 SJO-24 — 「정본」으로 지목된 절이 파생 문서와 반대를 말한 채 머지될 뻔했다)
- **검증 범위를 이슈에서 베끼지 않는다.** 이슈가 지정한 대상은 **산출물의 범위이지 영향의 범위가 아니다**. 구조를 바꾸는 결정(네비게이션·라우팅·공통 골격·토큰)은 그 구조를 물려받는 **전체 집합**에 대해 묻는다 — 4화면을 그렸어도 10화면 전부에 「도달 가능한가」를 묻는다. **근거로 열거한 목록은 결론과 대조한다**: N개를 열거했으면 결론이 N개를 다 덮는지 센다
- **「통과」가 수정 때문인지 환경의 관대함 때문인지 가른다.** 로컬·원격·CI는 프로덕션보다 **관대한 쪽으로** 다르기 쉽다 — 로컬 Node 24는 `require(esm)`를 허용하지만 Vercel Fluid 로더는 아니고(같은 24.x다), 느린 원격 5xx는 빠른 로컬 5xx가 드러내는 경합을 덮는다. 그래서 **하네스를 믿기 전에 하네스를 검증한다: 수정 전 코드가 그 하네스에서 반드시 실패해야 하고, 그 실패가 프로덕션의 실패와 같은 종류여야 한다.** 음성 대조가 없는 「통과」는 통과가 아니다. **격리했다는 주장도 같다 — 주입은 격리가 아니다**: 프로세스가 설정을 얻는 경로를 전부 세고(환경변수·cwd의 `.env`·인자) 각각을 막았는지 본다. `smoke.mjs`는 부모 env를 안 물려주면서 `cwd`의 `.env`를 그대로 읽어, 필수 키를 빼도 통과했다 (2026-09-06, SJO-14). 그리고 **조건 하나로 재지 않는다** — 정상·빠른 실패·형식이 어긋난 성공(2xx인데 JSON이 아님)을 매트릭스로 걸어 「N개 조건 중 M개 통과」를 센다. 후보 하나가 한 조건을 통과한 것은 근거가 아니다. **눈으로 보는 검증도 하네스다** — 브라우저·스크린샷·`getComputedStyle`이 그것이고, 키 이벤트가 도달하지 않는 도구와 내가 안 보는 속성은 「통과」를 공짜로 준다. 특히 **「아무 일도 안 일어난다」가 기대값인 검증**(차단·비활성·금지)은 하네스가 죽어도 똑같이 통과하므로, **「~하면 안 된다」를 재기 전에 「~하면 된다」를 먼저 재고** 눈이 아니라 계산값을 표로 센다 (2026-09-07, SJO-20 — 비활성 버튼이 활성과 똑같아진 회귀를 실화면으로 보고도 놓쳤다).
- **강제 장치는 우회 경로로 평가한다. 축이 둘이고 순서가 있다.** ① **발동 조건을 먼저 전수로 센다** — 그 장치를 부르는 트리거가 무엇이고 **무엇이 안 부르는가**. git 훅이면 `pre-commit`이 붙는 조작과 안 붙는 조작(`merge`·`rebase`·`cherry-pick`·`revert`·**서버 머지**)을 갈라 센다. 재는 법은 싸다 — **장치를 항상 실패(`exit 1`)로 바꿔 놓고 조작 종류별로 돌려 어느 것이 죽는지 본다.** ② 그 다음에 **발동한 뒤의 우회 경로**를 열거해 프로브 하나에 전부 담고 "N개 중 M개 검출"을 센다. 통과해야 할 정상 케이스도 같은 파일에 넣어 오탐까지 본다. **①을 건너뛰면 ②를 아무리 촘촘히 해도 장치가 안 붙는 경로가 통째로 사각지대로 남는다** (2026-09-08 SJO-43 — 7개를 세고 「구멍은 정확히 둘」이라 적었는데, 이 레포의 유일한 머지 전략인 merge commit이 훅을 아예 안 탔다). 위반 샘플 **하나**가 잡히는 것은 "규칙이 돈다"는 증거이지 "막힌다"는 증거가 아니다 — 문서에 "막는다"고 쓰기 전에 세어본다. **그리고 세어본 적 없는 수를 「정확히 N개」로 적지 않는다** — 열거를 문서에 함께 남겨야 다음 사람이 누락을 검산할 수 있다.
- **테스트와 음성 대조도 같은 장치다 — 뮤테이션 지점을 떠올리는 순서로 고르지 않는다.** 먼저 **가드를 열거하고**(가드 = 그것을 지우면 기능이 뚫리는 줄) 그 전부를 하나씩 지워 본 뒤 "N개 중 M개가 실패"를 센다. 한 기능에 가드가 둘이면(서비스의 선조회 + SQL의 조건) **뒤의 것이 진짜 방어선인 경우가 많다** — 앞의 것만 대조하고 "그 가드 확인함"으로 세지 않는다. 그리고 **스텁 경계 밖은 스텁 스펙이 검증하지 않는다**: 리포지토리를 스텁으로 둔 스펙은 SQL이 통째로 뚫려도 통과하므로, SQL이 방어선이면 쿼리를 `Db`를 받는 함수로 빼 `.toSQL()`로 절을 센다. **「~하면 실패한다」를 문서에 적기 전에 실제로 실패시켜 본다** — 2026-09-07 SJO-16에서 그 문장을 검증 없이 `docs/08`에 규약으로 적었고, 사실이 아니었다.
...
- **워크트리에서는 `gh pr merge`에 `--delete-branch`를 붙이지 않는다.** `main`이 메인 워크트리에 체크아웃돼 있어 gh의 정리 단계가 죽는데, 그때 **머지는 이미 끝나 있다** — 실패 메시지가 어느 단계 것인지 먼저 본다. 절차는 `/done` 6번
