...
- **외부 도구의 동작을 기억으로 단정하지 않는다.** Vercel·pnpm·ESLint·Drizzle 같은 도구의 설정이 무엇을 하는지는 **원문을 열어 확인한 뒤** 고친다. 관측 몇 건으로 세운 가설은 수정을 적용하기 전에 **반례를 먼저 찾는다** — 우연히 맞는 관측이 확증편향을 굳힌다. 검증 안 된 결론을 `docs/`에 커밋하지 않는다. 여기서 문서는 명세다.
