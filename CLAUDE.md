...
- **`exit 0`은 "통과"가 아니라 "오류 없음"이다.** 0건 실행과 0건 실패는 똑같이 0을 준다. 검증은 **실행 건수와 출력 전문**을 함께 본다 — 출력을 `tail`로 자르지 않고, 린터·테스트 probe는 레포 안에서 돌린다. 대상이 0건이면 통과가 아니라 **"대상 없음"으로 보고**한다.
