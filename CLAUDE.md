...
1. **시작** — **`/start` 스킬로만 착수한다**: 동시 2개·차단 확인 → `In Progress` + `me` → 이슈 DoD와 관련 docs/ 읽기 → 명세 공백 해소 → 계획
2. **구현** — 태스크 하나 끝날 때마다 체크박스를 `patch`로 `[x]`. 커밋 메시지에 `(SJO-N)`. 치명 영역(파서·채점·JWT 가드 — `08-testing`)을 건드리면 테스트를 같은 이슈에서 작성한다
3. **종료** — **`/done` 스킬로만 닫는다**: DoD 실검증 → 이슈 전체 diff 코드 리뷰(UI 변경 시 design-reviewer 추가) → 체크박스 → 검증 코멘트 → `Done`. 체크박스가 다 찼다고 닫지 않는다
