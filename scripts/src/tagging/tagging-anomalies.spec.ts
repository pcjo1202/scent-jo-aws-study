...
import {
  findTaggingAnomalies,
  MAX_CATEGORY_SHARE,
  MAX_UNTAGGED_SHARE,
} from './tagging-anomalies.ts'
...
describe('findTaggingAnomalies', () => {
  ...
  it('한 카테고리가 절반을 넘으면 잡는다 — 사전·롤업이 무너진 모양', () => {
    ...
    expect(anomalies.anomalyCounts['카테고리 편중']).toBe(1)
  })

  it('상한을 한 문항 넘긴 편중도 잡는다 — 경계에서 새지 않는다', () => {
    const limit = Math.floor(TOTAL * MAX_CATEGORY_SHARE)
    const questions = corpus((index) => topics(index <= limit ? ['컴퓨트'] : ['스토리지']))

    expect(findTaggingAnomalies(questions, KNOWN).anomalyCounts['카테고리 편중']).toBe(1)
  })
  ...
  it('상한과 정확히 같으면 통과한다', () => {
    ...
    expect(anomalies.anomalyCounts['카테고리 편중']).toBe(0)
  })
  ...
  it('카테고리가 4개인 문항을 잡는다', () => {
    ...
    expect(findTaggingAnomalies(questions, KNOWN).anomalyCounts['카테고리 상한 초과 문항']).toBe(1)
  })

  it('노트에 없는 카테고리를 잡는다 — 롤업이 값을 지어낸 모양', () => {
    const questions = corpus((index) => (index === 0 ? topics(['서버리스']) : topics(['스토리지'])))

    expect(findTaggingAnomalies(questions, KNOWN).anomalyCounts['노트에 없는 카테고리']).toBe(1)
  })
  ...
  it('같은 카테고리가 두 번 붙은 문항을 잡는다', () => {
    ...
    expect(findTaggingAnomalies(questions, KNOWN).anomalyCounts['카테고리가 중복된 문항']).toBe(1)
  })
  ...
  it('미태깅이 상한을 넘으면 잡는다 — 아무것도 안 붙으면 편중 상한은 발동하지 않는다', () => {
    ...
    expect(anomalies.overweight).toEqual([])
    expect(anomalies.anomalyCounts['미태깅 과다']).toBe(1)
  })

  it('미태깅이 상한과 같으면 통과한다', () => {
    const limit = Math.floor(TOTAL * MAX_UNTAGGED_SHARE)
    // 나머지를 둘로 갈라 편중 상한이 대신 발동하지 않게 한다.
    const questions = corpus((index) => {
      if (index < limit) return topics([], [])
      return topics(index % 2 === 0 ? ['스토리지'] : ['네트워크'])
    })

    const anomalies = findTaggingAnomalies(questions, KNOWN)

    expect(anomalies.untagged).toBe(limit)
    expect(total(anomalies)).toBe(0)
  })

  it('미태깅이 상한을 한 문항 넘기면 잡는다 — 경계에서 새지 않는다', () => {
    const limit = Math.floor(TOTAL * MAX_UNTAGGED_SHARE)
    const questions = corpus((index) => {
      if (index <= limit) return topics([], [])
      return topics(index % 2 === 0 ? ['스토리지'] : ['네트워크'])
