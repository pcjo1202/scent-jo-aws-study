 ...
 *
 * **이 절 혼자 막는 것이 아니다** — `finish`의 세션 잠금이 `UPDATE`를 대기시킨 뒤에야 이 절이
 * 판정할 것이 생긴다. `lockSessionQuery`의 `for update`를 지우면 ⑥도 함께 실패한다(같은 실측).
 * 「⑥은 이 절이 지킨다」가 아니라 **「잠금과 이 절의 곱이 지킨다」**다.
 *
 * **선조회의 `finishedAt` 검사는 반대로 잉여다** — 그것만 지워도 하네스 6조건이 전부 통과한다
 * (같은 실측). 잠금이 아니라서 창을 못 막고, 막는 것은 이 절이다. 그래도 두는 이유는
 * `finishSessionQuery`의 잉여 절과 **다르다** — 저것은 잠금 밖에서 불릴 미래의 마지막
 * 방어선이지만 이쪽은 지워도 SQL 절이 늘 남는다. 두는 이유는 셋이다: ① `deleteExam`과 같은
 * 모양이라 대칭이고 ② 쓰기를 보내기 전에 409를 내 왕복 하나를 아끼며 ③ `exams.spec.ts`가
 * 「종료된 세션이면 409이고 `updateCursor`를 부르지 않는다」로 그 순서를 계약으로 고정했다.
 * 0행이면 서비스가 409로 옮긴다.
 */
 ...
 * 절을 지워도 경합 하네스 6조건이 전부 통과한다(두 기기 동시 종료 포함). 진 쪽의
...
export class ExamsRepository {
   ...
   * 지우면 하네스 조건 ⑤가 3회 중 3회 실패한다(2026-09-08 실측). `updateCursorQuery`도 잠그지
   * 않는 경로라 같은 등급이고, 셋 중 `finishSession`의 절만 잉여다(`finishSessionQuery` 주석) —
   * 같은 SQL 조각이 셋인데 등급은 둘로 갈린다. 서비스의 선조회와 이 문장 사이가 열려 있어, A가 `finish`하는 동안 B가 `DELETE`를 보내면 B의
