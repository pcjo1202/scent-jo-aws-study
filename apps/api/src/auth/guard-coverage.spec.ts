 ...
 *
 * 여기서 세는 것은 이 파일이 만든 라우트다. 남은 구멍인 **누군가 실제 컨트롤러에
 * `@Public()`을 잘못 붙이는 것**은 `public-route-audit.spec.ts`가 센다 — AppModule의
 * import 그래프를 걸어 라우트를 전부 모으고 `IS_PUBLIC_KEY`가 `/health` 하나에만
 * 붙었는지 본다. 도메인 라우트 6개가 생긴 SJO-15에서 넣었다.
 */
@Controller('unmarked')
