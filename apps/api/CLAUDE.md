...
- `05`의 도출 쿼리는 **쓰는 모듈이 소유**한다: 풀이 상태 맵·오답 목록 → `progress`, 세션 채점 → `exams`
- **`stats`는 repository가 없다** — 카테고리별 정답률은 풀이 상태 맵을 접어서 만드는 것이라 `progress`의 쿼리를 그대로 쓴다. 같은 쿼리를 다시 쓰면 「미완료 exam 세션 제외」가 두 벌이 된다 (`docs/05` 「Nest 모듈 구성」)
...
이미 적용된 마이그레이션 파일은 **수정하지 않는다.** 새 파일만 추가한다. 스키마 제약(check·부분 유니크 인덱스)은 DB가 강제하게 두고 **서비스·리포지토리가 같은 검사를 다시 하지 않는다.**

**DTO는 예외다 — 경계에서 거르는 것은 중복이 아니라 다른 일이다.** DB 제약은 무결성을 지키고 DTO는 잘못된 요청을 400으로 만든다. DTO가 없으면 그 요청이 Postgres까지 가서 23514·23505로 죽고 **500이 나간다** — 클라이언트는 자기 잘못을 서버 장애로 읽는다. 그래서 DTO가 DB check의 값을 되적는 자리가 있다: `create-attempt.dto.ts`의 `@ArrayMaxSize(3)`(`attempts_selected_size`), `update-exam.dto.ts`의 `@Max(64)`(`exam_sessions_cursor`). 되적을 때는 **상수의 출처를 주석으로 남긴다** — 값이 갈리면 400과 500의 경계가 조용히 움직인다.
