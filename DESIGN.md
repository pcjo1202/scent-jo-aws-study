...
4. **기다리게 하는 모션은 방해다.** 1019번 반복할 화면이 바뀌기를 기다리게 하지 않는다 — 누른 것에 대한 응답은 대기가 아니다
...
**전환을 두는 자리와 두지 않는 자리는 duration이 아니라 「기다리게 하는가」로 갈린다.** 문항 전환과 채점 결과는 사용자가 **다음 화면을 기다리는** 자리다. 문항 전환은 문항당 정확히 1회 발생하므로 **200ms를 1019번 기다리면 3분 24초다.** 채점 결과는 답을 제출한 직후, 집중이 가장 높은 순간이라 거기에 애니메이션을 끼우면 매번 흐름이 끊긴다. **둘 다 즉시 표시한다.**

남은 둘은 **누른 것에 대한 응답**이다. 상태 레이어는 `opacity`라 아무것도 가리지 않고, 아코디언 펼침은 문항당 0~1회이며 누른 즉시 반응이 시작된다 — 기다림이 아니라 눌렀다는 확인이다. 원칙 4가 금지하는 것은 이쪽이 아니다.
