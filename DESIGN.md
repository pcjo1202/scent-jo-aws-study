...
**정보 구조는 폭에 따라 갈리지 않는다.** 어느 폭에서도 같은 것을 같은 순서로 보여준다. 갈리는 것은
위 표의 마지막 두 행과, 거기서 파생되는 둘뿐이다 — `expanded`에서는 보조 패널이 상시 열려 있으므로
**앱바 우측 액션 버튼이 사라지고**, 키보드가 있는 폭이므로 **선택지에 키 힌트가 붙는다**
(「선택지 · 키보드」). `expanded` 전용 구조를 새로 만들면 모든 화면을 두 번 설계하고 두 번 검증하게
된다 (2026-09-02, SJO-44).

**`medium`(600–839px)은 `compact`의 규칙을 따르고 화면 여백만 24px이다.** 보조 패널을 열기에는
읽기 칼럼이 남지 않는다 — 320 + 672 = 992px이 필요하다.
...
**단, 가변 폰트로 불러오지 않고 개별 SVG를 인라인한다.** 필요한 아이콘이 12개뿐인데 폰트 파일을 받는 것은 낭비다. 폰트 로딩 전 아이콘이 사라지는 구간(FOIT)도 없앨 수 있다.
...
M3 컴포넌트 라이브러리를 쓰지 않으므로, 이 앱에 실제로 있는 일곱 개만 정의한다. 전부 위에서 정한 토큰으로 조립한다.
...
하단 탭 바를 두지 않는 이유가 셋이다. `compact`의 화면 하단 64px는 「하단 액션 바」가 이미 점유해
...
`compact`에서는 전체 화면 시트, `expanded`에서는 좌측 320px 상시 패널이다. **내용은 같고 담는 그릇만
다르다.** 시트는 화면 폭 전체라 `corner-none`이고, 헤더는 앱바 규격을 쓰되 `arrow_back`으로 닫고
제목을 좌측 정렬하며 우측에 「모두 해제」 텍스트 버튼을 둔다.

**필터 패널** (`/study`·`/review`) — `docs/02-features.md` 「필터」의 네 그룹을 그 순서로 쌓는다.
그룹마다 `title-small` 제목 + 칩 목록이고, 그룹 사이 24px이다.
...
대시보드에만 있는 넷이다. 나머지 화면은 위 컴포넌트로 조립된다.
...
| 오답 진입 행 | 최소 48px · 위아래 `outline-variant` 1px · 우측 `chevron_right` |
...
**색을 쓰지 않는다.** 빈 상태는 오류가 아니다 — `/review`의 「복습할 오답이 없다」에 경고색을 쓰면
0건이 실패로 읽힌다 (`docs/02-features.md` 「빈 상태」).
