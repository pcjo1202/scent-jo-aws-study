...
것은 그것을 담는 그릇뿐이다 — 위 표의 네 행(본문 폭 · 액션 위치 · 보조 패널)과, 거기서 파생되는 둘 — `expanded`에서는 보조 패널이 상시 열려 있으므로
**앱바 우측 액션 버튼이 사라지고**, 키보드가 있는 폭이므로 **선택지에 키 힌트가 붙는다**
(「선택지 · 키보드」). `expanded` 전용 구조를 새로 만들면 모든 화면을 두 번 설계하고 두 번 검증하게
된다 (2026-09-02, SJO-44).

**보조 패널은 992px부터 열린다** — 패널 320 + 읽기 칼럼 672다. 그래서 폭에 따라 셋으로 갈린다.

| 폭 | 본문 | 보조 패널 | 액션 |
|---|---|---|---|
| < 600 (`compact`) | 전체 폭 · 여백 16 | 전체 화면 시트 | 하단 고정 |
| 600–991 (`medium` + `expanded` 아래쪽) | 전체 폭 · 여백 24 | 전체 화면 시트 | 하단 고정 |
| ≥ 992 | 읽기 칼럼 중앙 | 좌측 320px 상시 | 읽기 칼럼 하단, 흐름 안에 |

**992px은 「Window size class」의 경계가 아니다.** 그 표는 여백을 정하고, 이 표는 배치를 정한다 —
`expanded`(≥840)의 아래쪽 152px 구간은 여백만 24px이고 배치는 `compact`와 같다. 경계를 하나로
합치려고 `expanded`를 992로 올리지 않는다. 여백이 갈리는 지점과 칼럼이 갈리는 지점이 실제로 다르다.
...
| 대시보드 카드 (진도 · 최근 모의고사) | `surface-container-low` |
| 진행 중 모의고사 이어가기 배너 | `surface-container` |
...
M3 컴포넌트 라이브러리를 쓰지 않으므로, 이 앱에 실제로 있는 아홉 개만 정의한다. 전부 위에서 정한 토큰으로 조립한다.

## 공통 헤더·네비게이션

9개 화면이 전부 물려받는 골격이다. 여기서 정하지 않으면 화면마다 즉흥으로 갈린다.

### 네비게이션은 허브-스포크다

**`/` 대시보드가 홈이고, 상시 네비게이션을 두지 않는다.** 나머지 화면은 `/`에서 들어가고 앱바
좌측 뒤로가기로 나온다. 깊이는 1이다.
...
|---|---|---|---|
| `/` | `AWS SAA-C03 학습` | — | — |
| `/study` | `순차 12 / 1019` | `filter_list` + 활성 개수 배지 | ✓ |
| `/review` | `오답 복습 3 / 27` | `filter_list` | ✓ |
...
좁은 폭에서는 전체 화면 시트, 992px부터는 좌측 320px 상시 패널이다 (「화면 구성」의 폭 표).
**내용은 같고 담는 그릇만 다르다.** 시트는 화면 폭 전체라 `corner-none`이고, 헤더는 앱바 규격을 쓰되
`arrow_back`으로 닫고 제목을 좌측 정렬한다.
...
`compact` 시트에만 하단에 결과 수를 담은 주 버튼(「N문제 보기」)을 두고, `expanded` 패널은 상시
반영이므로 `body-small`로 개수만 적는다. 「모두 해제」 외의 적용 버튼을 두지 않는다.

**문제 이동 그리드** (`/exam/[id]`) — 65칸을 6열 격자로 놓는다. 칸 높이 36px · `corner-small` ·
`label-medium` · 간격 8px.
...
`outline-variant`는 `surface` 대비 1.61:1이라 36px 칸에서 사실상 안 보인다 (「대비 검증」 § 각주).

칸 안은 숫자뿐이고 아이콘을 넣지 않는다. 색을 잃어도 남는 구분은 **테두리 두께**(2px = 현재)이고,
답함/안 함의 개수는 패널 하단에 `body-small`로 병기한다 — 「N문제 답함 · M문제 남음」.
...
**첫 문제에서 「이전」은 비활성이다. 숨기지 않는다.** 자리가 사라지면 「제출」이 옆으로 움직여서
같은 자리를 1019번 누르던 손이 매번 어긋난다. 비활성은 `state-disabled-content`(0.38)다.
...
어디로 갈지 달라지는데, 허브-스포크라 깊이가 1이므로 고정 링크로 두어도 잃는 것이 없다.
...
카드 안 요소 순서는 미채점과 채점 후가 같아야 제출 전후로 줄이 밀리지 않는다.

```
미채점    [input 표식] [키 A~F] [선택지 텍스트 ......] [키 힌트(expanded만)]
채점 후   [키 A~F]     [선택지 텍스트 ...............] [정오 아이콘]
```

채점 후에는 `input`이 사라지고 **그 자리를 아이콘이 받지 않는다** — 아이콘은 오른쪽 끝이다. 왼쪽
표식 자리를 정오 아이콘이 이어받으면 「눌러서 고르는 것」과 「판정 결과」가 같은 자리를 쓰게 된다.
...
**미결**: 배경이 없어졌으므로 `compact`에서 스크롤되는 지문이 버튼 뒤로 지나간다. `primary` 채움인
...
대시보드에만 있는 여섯이다. 나머지 화면은 위 컴포넌트로 조립된다.

| 요소 | 규격 |
|---|---|
| 전체 진도 카드 | `surface-container-low` · `corner-medium` · 패딩 16px. 「전체 진도」 `body-small` → `247 / 1019` (`headline-small`, 분모는 `body-large`·`on-surface-variant`) → 8px 트랙 |
| 이어풀기 | 주 버튼 규격 그대로 (`primary` · `corner-full` · 56px). 폭은 전체. `/study`로 간다 |
| 진행 중 모의고사 이어가기 | `surface-container` · `corner-medium` · 최소 48px · 문구 `body-medium` + 우측 텍스트 버튼. **상태 배너가 아니다** — 그쪽은 로딩·오류·저장 대기 셋 전용이다 |
| 카테고리별 정답률 막대 | 라벨(`body-small`·88px) · 8px 트랙(`surface-container-high`) · 값(`body-small`·우측 정렬) |
| 오답 진입 행 | 최소 48px · 위아래 `outline-variant` 1px · 우측 `chevron_right` · 문구 「오답 **N**문제」(`body-large`) |
| 최근 모의고사 | `title-small` 제목 + 최근 3개. 한 행은 최소 36px · 좌측 날짜(`body-medium`·`on-surface-variant`) · 우측 `47 / 65`(`body-medium`, 점수만 500). 행을 누르면 `/exam/[id]/result`로 간다 |
...
대시보드가 예외인 이유: 시도가 0건이어도 진도 카드(`0 / 1019`)와 이어풀기는 그대로 있고, 사라지는
것은 정답률 막대뿐이다. 화면이 비지 않으므로 가운데로 모을 것이 없다.

### 빈 상태에서 골격은 어떻게 되나

문제가 0건이면 앱바의 진행 숫자와 하단 액션이 가리킬 대상이 없다.
...
되돌릴 수 없는 것을 확인받을 때만 쓴다 — 진행 중 모의고사 포기, 세션 종료, `content_version` 409
(`docs/02-features.md` 「API 오류의 화면 표현」). 그 밖에는 쓰지 않는다.
...
**본문 영역의 첫 블록**이다. 앱바 안이 아니다 — 앱바는 56px 고정이라 배너를 넣으면 제목이 밀린다.
`docs/02-features.md` 「백엔드 요청 실패」의 *"상단에 「저장 대기 N건」을 표시한다"* 가 이 자리다.

「저장 대기」는 제출이 일어나는 세 화면(`/study` · `/review` · `/exam/[id]`)에만 뜬다. 로딩·오류는
그 화면의 데이터를 기다리거나 못 받은 자리에 뜬다.
