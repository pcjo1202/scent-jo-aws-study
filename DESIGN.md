...
| 종류 | 아이콘 | 문구 | 라이브 리전 | 액션 |
|---|---|---|---|---|
| 로딩 | 없음 | **불러오는 중…** | `aria-live="polite"` | 없음 |
| 오류 | `error` | **불러오지 못했다** + 무엇을 못 했는지 | `role="alert"` | **호출부가 정한다** |
| 저장 대기 | `cloud_off` | **저장 대기 N건** | `aria-live="polite"` | 없음 |
...
`aria-live`가 붙은 요소는 **변경 시점에 이미 DOM에 있어야** 낭독된다. 배너를 새로 마운트하면서 그 배너에 `aria-live`를 다는 것은 늦다. 상태를 표시하는 쪽이 컨테이너를 항상 렌더하고 배너는 그 안에서 교체된다.

`role="alert"`는 삽입 시점에도 동작하지만, 두 경로를 섞으면 어느 배너가 왜 낭독되는지가 코드에서 사라진다. **컨테이너를 항상 두는 쪽으로 통일한다.**
