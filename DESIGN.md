...
**1. 행간을 키운다.** 계열마다 목표 비율을 정하고 **가장 가까운 짝수 px**로 맞춘다.
...
**벌린 것은 `body`뿐이다** — M3의 1.5에서 1.75로. 긴 지문에서 1.5는 벽처럼 보인다. `body-large`가 24px → **28px**이 되는 것이 여기서 나오고, **이 앱에서 가장 중요한 단일 수치다.** 나머지 셋은 M3 원본 비율에 그대로 붙는다 (`title-medium` 1.5 · `headline-small` 4/3 · `label-medium` 4/3).
...
**3. 크기를 +1px 한다 — `title`·`body`·`label` 계열에만.** 위 인용의 「Title~Caption」이 그 셋이다. 작은 크기에서 한글 획이 뭉개지는 것을 막기 위함이고, `display`·`headline`은 이미 충분히 커서 필요가 없다.

**예외는 `body-large` 하나다.** 16px이 이미 편안한 하한이라 그대로 둔다 — 이 역할은 크기가 아니라 **행간**으로 읽기 편함을 산다 (위 1.75).

**경계는 크기가 아니라 계열이다.** M3에서 `title-medium`과 `body-large`는 **둘 다 16px**인데 앞은 +1을 받고 뒤는 안 받는다. 「16px 미만」·「16px 이하」 같은 크기 경계로는 어느 쪽도 「우리 스케일」을 재현하지 못한다.
