...
것은 그것을 담는 그릇뿐이고, 전부 다음 다섯이다 — 위 표에서 셋(본문 폭 · 액션 위치 · 보조 패널의
그릇), 거기서 파생되는 둘 — `expanded`에서는 보조 패널이 상시 열려 있으므로
**앱바 우측 액션 버튼이 사라지고**(패널이 이미 열려 있다), 키보드가 있는 폭이므로 **선택지에 키
힌트가 붙는다**(「선택지 · 키보드」). 시트의 「N문제 보기」 버튼이 패널에서 개수 표시가 되는 것도
...
패널이 열리는 폭을 992로 올리면 840–991 구간에서 필터에 닿을 방법이 사라진다. 우측 액션 버튼이
`expanded`에서 사라지기 때문이다.
...
| 크기 | 최소 18×18px · 좌우 패딩 4px · `corner-full` |
...
대시보드에만 있는 일곱이다. 나머지 화면은 위 컴포넌트로 조립된다.
...
`/` 행이 정본이다 — 정답률 막대와 그 아래 줄, 오답 진입 행, 최근 모의고사, 이어가기 배너가 빠진다.
