...
**이 표를 손으로 계산하지 않는다 — `pnpm tokens:verify`가 판정한다.** 이 절과 「팔레트」·「색으로만 갈리지 않는 쌍」의 수치는 전부 `tokens.css`를 파싱해(`--sys-color-*` → `--ref-*` → hex) 다시 계산한 값과 대조되고, 하나라도 어긋나면 명령이 실패한다. 팔레트를 바꿨으면 그 명령이 짚어 주는 값으로 고친다.

계산기 자체는 알려진 기지값(`#6750A4`+흰 6.44 · `#B3261E`+흰 6.54 · `Lab(#FF0000)`)을 재현하는지로 매 실행마다 검증한다 — 그것부터 어긋나면 나머지 수치를 보지 않는다. 왜 검증만 있고 팔레트 **생성**은 스크립트에 없는지는 `docs/10-conventions.md` 「토큰 검증」.
...
| `primary-container` / `error-container` (Dark) | 23.6 | 5.5 | 67.8 |
| `primary` / `error` (Light) | 28.0 | 5.6 | 73.4 |
...
**42쌍 중 JND 미만이 1건이다.** `error` × `correct`는 재생성 대상이 아니어서 보라 시절과 값이 같다 — 정오 판정 자체는 팔레트 교체로 나빠지지 않았다.
...
- **`primary`는 정오를 나르지 않는다.** 「색 사용 규칙」이 `primary`를 링크와 주 버튼에만 묶어두므로 `error`와 의미가 경쟁하지 않는다. 형태·위치도 다르다(알약 버튼 vs 전체 폭 배너). 이 잠금이 풀리면 위 5.6이 문제가 된다 — JND는 넘지만 정상 시각의 28.0에서 5분의 1로 눌린 값이라, 정오처럼 **틀리면 안 되는 구분**을 얹을 여유가 아니다
...
- **정답 선택지를 `correct-container` 채움으로만 표시하지 않는다.** 선택된 선택지가 `secondary-container`라서, 적록색약 사용자에게 "내가 고른 것"과 "정답"이 같은 면색이 된다. 아이콘·텍스트 병기가 필수이고, 면 채움 대신 `correct` 테두리를 쓰면 색으로도 갈린다(색각 최악 48.5)
...
서는데**, `primary`×`error`는 적록색약 ΔE 5.6으로 정상 시각 28.0의 5분의 1이다 (「색으로만 갈리지
않는 쌍」). 그 표가 잠금의 근거로 든 *"형태·위치도 다르다"* 가 대시보드에서 깨진다. 진도는 좋고 나쁨이
...
는 그대로다 — 안 고른 정답은 여전히 `correct` 테두리다 (색각 최악 ΔE 48.5). (2026-09-02, SJO-44)
