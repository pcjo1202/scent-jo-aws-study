...
| 앱바와의 관계 | 앱바를 덮는다 | **앱바 아래에서 시작한다.** 앱바는 전폭이다 |
| 헤더 | 앱바 규격 · `arrow_back`으로 닫고 제목 좌측 정렬 | **없다.** 그룹 제목이 이미 구조를 나른다 |
| 「모두 해제」 | 헤더 우측 (필터 시트만) | 하단 개수 줄 우측 (필터 패널만) |
| 하단 | 주 버튼 「N문제 보기」 | `body-small` 개수 |

배경이 `surface-container-low`인 이유는 두 방향이다 — 본문(`surface`)과 갈려야 하고, 칩이
`surface-container-high`라 그보다 낮은 단계여야 칩이 면 위로 떠 보인다.
...
오른쪽 정렬이라 「이전」이 사라져도 주 버튼은 안 움직인다 — 자리 흔들림은 이유가 아니다. 채점 후에
「이전」이 실제로 없어지는 것도 같은 근거다(위 표): 그 상태에서 갈 곳은 다음 문제뿐이라 되돌아가기가
성립하지 않는다. **「숨기지 않는다」는 첫 문제에만 걸린 규칙이다.**
