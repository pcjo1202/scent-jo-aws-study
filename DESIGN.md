...
**대신 카드의 상자는 같다** — `min-height` 48px · 패딩 12/16 · 간격 12px · `corner-medium`이
그대로다. 카드 높이와 카드 사이 간격이 유지되므로 목록 전체의 리듬은 흔들리지 않는다.
...
지문에서 ①까지의 간격은 **24px**이다. 그 아래 ①↔안내↔선택지, 선택지↔④는 「간격」의
해설 블록 사이 16px을 따른다.
