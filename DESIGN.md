...
**이 표를 손으로 계산하지 않는다 — `pnpm tokens:verify`가 판정한다.** 이 절과 「팔레트」·「색으로만 갈리지 않는 쌍」의 표 전체, 그리고 그 값을 인용하는 다른 절의 산문 수치를 `tokens.css`에서(`--sys-color-*` → `--ref-*` → hex) 다시 계산한 값과 대조한다. 표에서 행이 빠지거나 새 역할이 분류 없이 들어오는 것도 함께 잡는다. 하나라도 어긋나면 명령이 실패하므로, 팔레트를 바꿨으면 그 명령이 짚어 주는 값으로 고친다.

계산기 자체는 알려진 기지값(`#6750A4`+흰 6.44 · `#B3261E`+흰 6.54 · `Lab(#FF0000)`)을 재현하는지로 매 실행마다 검증한다 — 그것부터 어긋나면 나머지 수치를 보지 않는다. **다만 기지값은 대비와 Lab까지만 덮는다** — 색각 시뮬레이션에는 대응하는 기지값이 없어 위생 검사가 유일한 음성 대조다. 그 한계와, 왜 검증만 있고 팔레트 **생성**은 스크립트에 없는지는 `docs/10-conventions.md` 「토큰 검증」.
...
표에 싣는 것은 **색각 최악이 낮은 4쌍과 `error` × `correct` 계열 4행**이다. 값이 바뀌면 이 기준을 다시 적용한다 — `pnpm tokens:verify`가 표의 구성을 이 기준과 대조한다.
...
**「보라 시절」 열은 검증 대상이 아니다.** 교체 전 팔레트가 이 레포에 남아 있지 않아 `pnpm tokens:verify`가 재계산할 수 없다. 나머지 두 열은 매번 재계산해 대조한다.
...
- **정답 선택지를 `correct-container` 채움으로만 표시하지 않는다.** 선택된 선택지가 `secondary-container`라서, 적록색약 사용자에게 "내가 고른 것"과 "정답"이 같은 면색이 된다. 아이콘·텍스트 병기가 필수이고, 면 채움 대신 `correct` 테두리를 쓰면 색으로도 갈린다(`correct` × `secondary-container` 색각 최악 48.5)
...
| `/notes` 비교노트 카드 | `surface-container-low` + `outline` 1px |
...
| 오답 | `cancel` | **filled** |
| 등장 서비스 칩 펼침 · 비교노트 카드 펼침 | `expand_more` | outlined |
...
| `label-large` | 15px | 20px | 500 | 0 | 버튼, 테마 라디오, 비교노트 구성원 이름 |
...
1.25:1(Light)이라 면색만으로는 경계가 서지 않는다. 면 채움은 색이 살아 있을 때 정답을 먼저 읽히게
...
는 그대로다 — 안 고른 정답은 여전히 `correct` 테두리다 (`correct` × `secondary-container` 색각 최악 ΔE 48.5). (2026-09-02, SJO-44)
...
| 속성 | 값 |
|---|---|
| 정렬 | 본문 영역 **수직·수평 가운데**. 예외가 둘이다 — 대시보드는 상단 정렬을 유지하고, `/notes` 검색 0건은 화면이 아니라 한 구역만 비운다 (둘 다 아래) |
...
**화면이 아니라 한 구역만 비는 경우가 하나 있다 — `/notes`의 검색 0건.** 검색이 한줄노트에만
걸리므로(`docs/02-features.md` 「`/notes` 암기 노트」) 비교노트는 그대로 남고, 문구는 카테고리
그룹이 서던 자리에만 든다. 위 표에서 **「정렬」만 어긋난다** — 본문 영역이 아직 비어 있지 않아
수직 가운데의 기준이 없다. 나머지 행(문구 `title-medium`·액션 없음·`error` 금지)은 그대로다.
**액션을 두지 않는 이유**는 되돌릴 대상이 화면 안의 입력이기 때문이다 — 「필터 해제」 버튼이
필요했던 `/study` 필터 0건과 달리, 여기서는 지우면 그만인 검색어가 바로 위에 보인다.

**여백은 이 자리에서 두 겹이 된다** — 화면 여백 16px 안에 빈 상태 자신의 24/16 패딩이 들어가
좌우가 `compact`에서 32px가 된다. 32는 「간격」이 열거한 값이고, 가운데 정렬 한 줄이라 그 안쪽
경계에 놓이는 것이 없다. **위쪽은 겹치지 않게 한다** — 빈 상태 자신의 24px을 쓰고 바깥에서
다시 주지 않는다. 더하면 40px가 되어 열거 밖으로 나간다 (2026-09-09, SJO-25).
