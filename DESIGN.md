...
4. **기다리게 하는 모션은 방해다.** 1019번 반복할 화면의 **내용**이 바뀌기를 기다리게 하지 않는다 — 누른 요소가 **제자리에서** 반응하는 것은 대기가 아니다
...
| M3 Expressive의 shape morphing·표현적 모션 | 목적이 감정적 임팩트다 (위 절) — 원칙 4와도 충돌한다 |
...
| `disabled` | M3는 컨테이너 0.12 · 내용 0.38 | **이 앱은 내용 0.38만 쓴다** (아래 「이 앱에서」) |
...
| **disabled** | **내용만 0.38로 낮춘다. 면색과 테두리는 그대로 둔다** |

`selected`를 상태 레이어로 처리하지 않는 이유: 선택은 일시적 피드백이 아니라 **지속되는 의미**다. 0.08~0.12 오버레이로는 "내가 이걸 골랐다"가 충분히 보이지 않는다.

**`disabled`를 요소 전체에 걸지 않는 이유**는 2026-09-07 `/study` 실화면에서 나왔다 (SJO-20).
복수정답에서 필요 개수를 채우면 나머지 선택지가 비활성이 되는데, 요소째 0.38로 낮추면 「선택지 ·
기본」이 *카드 경계가 사실상 이 선 하나에 의존한다*고 못 박은 `outline` 테두리가 사라지고 본문도
읽히지 않았다. **비활성은 「지금은 못 누른다」이지 「읽을 필요 없다」가 아니다** — 초과 선택을
막는 장치이지 정보를 숨기는 장치가 아니다 (`docs/02-features.md` 「선택 규칙」). 위 표의
*컨테이너 0.12 · 내용 0.38* 중 **컨테이너 쪽은 쓰지 않는다** — 카드가 이미 낮은 면색이라 거기서
0.12를 더 빼면 같은 문제가 약하게 되돌아온다.
...
| 대시보드 카드 (진도 · 최근 모의고사) · 결과 화면 점수 카드 | `surface-container-low` + `outline` 1px |
...
**로딩에는 아이콘을 두지 않는다.** Material Symbols의 `progress_activity`는 회전을 전제한 글리프인데, **로딩이 바로 「Motion」이 전환을 두지 않기로 한 그 자리**다 — 다음 내용을 기다리는 자리. 회전을 줄 수 없으니 멈춰 있는 호(弧) 하나만 남아 아무 의미도 나르지 않는다. 로딩 배너는 문구가 상태를 다 말한다 (「Components · 상태 배너」).
...
`medium` 이상과 `emphasized` 계열은 쓰지 않는다. M3 Expressive의 표현적 모션은 감정적 임팩트를 위한 것인데, 이 앱은 같은 동작을 1019번 반복한다.

**전환을 두는 자리와 두지 않는 자리는 duration이 아니라 「기다리게 하는가」로 갈린다.** 문항 전환과 채점 결과는 사용자가 **다음 내용을 기다리는** 자리다. 문항 전환은 문항마다 발생하므로 **200ms를 1019번 기다리면 3분 24초이고, 되돌아가면 더 는다.** 채점 결과는 답을 제출한 직후, 집중이 가장 높은 순간이라 거기에 애니메이션을 끼우면 매번 흐름이 끊긴다. **둘 다 즉시 표시한다.**

남은 둘은 **누른 요소가 제자리에서 반응하는 것**이다. 상태 레이어는 눌린 카드 위에서 `opacity`만 바뀌어 아무것도 가리지 않고, 아코디언은 누른 `<summary>` 바로 아래가 열린다 — 다음 내용을 기다리는 것이 아니라 눌렀다는 확인이다. 원칙 4가 금지하는 것은 이쪽이 아니다.

`prefers-reduced-motion: reduce`에서는 남은 두 전환을 전부 제거한다.
...
**세 가지**를 바꾼다. 셋 다 Google 자신의 CJK 가이드에 근거한다.

> CJK는 글자가 크고 조밀해서 영문과 같은 디자인 의도를 달성하려면 행간을 영문보다 키워야 하고, 그러지 않으면 두 줄 사이에서 글자가 잘릴 수 있다. Title~Caption은 폰트 크기도 영문보다 1px 크게 잡는다.

**1. 행간을 키운다.** **계열**(`display`·`headline`·`title`·`body`·`label` 다섯)마다 목표 비율을 정하고, **아래 3의 +1px을 적용한 뒤의 크기**에 곱해 **가장 가까운 짝수 px**로 맞춘다. M3 원본 크기에 곱하면 8역할 중 6이 어긋나고, 그중 둘은 짝수 둘의 정중앙에 떨어져 값이 정해지지도 않는다.
...
| `title` | 1.5 |
| `headline` · `label` | 4/3 |
| `display` | 쓰지 않으므로 정하지 않는다 |

**짝수인 것은 M3 원본 행간 15개가 전부 짝수이기 때문이다.** 정수로 반올림하면 `body-small`(22.75 → 23)과 `label-medium`(17.33 → 17)이 「우리 스케일」과 어긋난다.

**계열 목표를 M3에서 옮긴 것은 `body`뿐이다** — M3의 1.5 · 1.4286 · 1.3333 셋을 전부 **1.75**로 통일했다. 긴 지문에서 1.5는 벽처럼 보인다. `body-large`가 24px → **28px**이 되는 것이 여기서 나오고, **이 앱에서 가장 중요한 단일 수치다.**

나머지 셋은 M3의 대표값을 그대로 계열 목표로 삼는다. 다만 **계열을 하나의 비율로 묶은 대가**가 두 역할에 남는다 — `title-small`은 1.4286 → 1.5로 벌어져 행간이 20 → **22px**이 되고, `label-large`는 1.4286 → 4/3으로 좁아지지만 크기가 +1px 되어 행간은 **20px 그대로**다.
...
**3. 크기를 +1px 한다 — `title`·`body`·`label` 계열에만.** 위 인용의 「Title~Caption」은 M2 시대 명칭이고, M3에서 그 범위에 대응하는 것이 이 셋이다 (`display`·`headline`이 그 위). 작은 크기에서 한글 획이 뭉개지는 것을 막기 위함이고, `display`·`headline`은 이미 충분히 커서 필요가 없다.

**예외는 `body-large` 하나다.** 16px이 이미 편안한 하한이라 그대로 둔다 — 이 역할은 크기가 아니라 **행간**으로 읽기 편함을 산다 (위 1.75).

**크기 경계로는 재현되지 않는다.** M3에서 `title-medium`과 `body-large`는 **둘 다 16px**인데 하나는 17px, 하나는 16px이 된다. 「16px 미만」·「16px 이하」 어느 쪽으로도 이 둘을 갈라낼 수 없다 — 규칙은 **계열 + 예외 하나**다.
...
로그인 후 9개 화면이 물려받는 골격이다. 여기서 정하지 않으면 화면마다 즉흥으로 갈린다.
...
**깊이는 대부분 1이지만 `/exam` 계열만 깊고, 그 최대가 3이다.** 뒤로가기는 항상 **라우트의 한
단계 위**로 간다.
...
| `/exam/[id]/result/[N]` | **`/exam/[id]/result`** |
| `/` | 없다 (허브) |
| `/login` | 없다 (앱바 자체를 두지 않는다) |

**문항 리뷰가 라우트인 이유가 이 표다.** 결과 요약 위에 덮는 상태로 두면 앱바 좌측이 「닫기」와
「`/exam`으로」 사이에서 **어떻게 들어왔는지에 따라** 갈리는데, 그것이 아래 「뒤로가기는 히스토리가
아니라 라우트가 정하는 고정 링크다」가 막는 상태다. 라우트로 두면 한 줄이 더 늘 뿐 규칙은 그대로다
(2026-09-08, SJO-24).
...
| `/exam/[id]/result` | `모의고사 결과` | — | — |
| `/exam/[id]/result/[N]` | `결과 12 / 65` | — | — |

**결과 두 화면에는 우측 액션도 진행 바도 없다.** 문항을 오가는 그리드가 **요약 화면의 본문 안에**
있어 열고 닫을 것이 없고(「문제 이동 그리드」), 진행 바는 문제 풀이 3화면만이라는 위 「앱바」의
규칙을 넓히지 않는다 — 읽는 화면이지 푸는 화면이 아니고 위치는 제목의 `12 / 65`가 이미 준다.
그 자리는 다른 진행 바 없는 화면과 같이 `outline-variant` 1px이 받는다.
...
| 서비스 | 검색 입력(`corner-extra-small`·`outline` 1px·48px·**`body-large`**) + 선택된 것만 칩으로. **16px 미만이면 iOS Safari가 포커스에서 화면을 확대한다** — 「우리 스케일」에서 16px은 `body-large` 하나다 |
| 정답 개수 | 칩 2개 |
| 풀이 상태 | 칩 3개 |
...
320px 패널에서 6열은 38.7px이라 성립하지 않는다. 실측(2026-09-08, SJO-23)은 **패널 4열 58px ·
시트 6열 50.5px**이고 둘 다 48을 넘는다 — 열 수가 아니라 칸 크기가 규칙이므로 이 차이는 위반이
아니다.

**패널이 5열이 아닌 이유는 1px이다.** 320px에 패딩 24×2를 빼면 272px이고 5열(48×5 + 8×4)이 정확히
272px이지만, 패널의 우측 경계 1px이 `border-box` 안에 들어 가용폭이 271px이 된다. 계산상 딱 맞는
값은 실제로는 안 맞는다.
...
**결과 화면(`/exam/[id]/result`)은 같은 그리드를 본문 안에 놓는다.** 그릇이 다르다 — 시트도 패널도
아니고 본문 흐름의 한 블록이라 열고 닫을 것이 없고, 그래서 그 화면의 앱바 우측이 비어 있다
(「화면별 우측 액션」). 칸 크기·간격·shape·타입스케일은 위와 같고 **열 수는 여기서도 폭이 정한다**.

**세션이 끝나면 정오를 감출 이유가 사라지므로 칸이 정오를 나른다.** 위 표가 `correct`·`error`를
금지한 근거는 「진행 중에는 정오를 표시하지 않는다」이고, 그 전제가 종료 후에는 성립하지 않는다.

| 칸 (결과) | 배경 | 테두리 |
|---|---|---|
| 정답 | `correct-container` / `on-correct-container` | 투명 1px |
| 오답 | 투명 / `on-surface` | **`error` 2px** |
| 미응답 | 투명 / `on-surface-variant` | `outline` 1px |

**오답을 `error-container` 채움으로 하지 않는다.** 그러면 정답과 **면색으로만** 갈려
「색을 전부 걷어내도 구분이 남아야 한다」(Foundations · 색 단독 전달 금지)를 어긴다 —
`error-container` × `correct-container`는 색각 최악 8.8~15.3으로 JND는 넘지만, 그 원칙은 색각이
아니라 **흑백**을 기준으로 쓴 것이다. 「안 고른 정답」을 `correct` 2px 테두리로 둔 것과 같은 해법을
쓴다: 면 채움 하나를 테두리로 바꾸면 **테두리(없음 / `error` 2px / `outline` 1px)** 가 셋을 가른다.
**흑백에서 실제로 일하는 축은 테두리 하나다** — `correct-container`는 `surface` 대비 그레이스케일
1.26:1(Light)이라 면색만으로는 경계가 서지 않는다. 면 채움은 색이 살아 있을 때 정답을 먼저 읽히게
하는 보조축이지 분리의 근거가 아니다. 분리의 근거는 진행 중 그리드가 이미 쓰던 것과 같아서
(테두리 유무와 두께) 축을 새로 만들지 않는다.

칸의 `aria-label`은 「12번 문제 정답」·「12번 문제 오답」·「12번 문제 미응답」이다. 하단 개수 줄은
「N문제 정답 · M문제 오답」이고, 미응답이 0보다 크면 「· K문제 미응답」을 잇는다 — 미응답은 오답으로
채점되지만(`docs/02-features.md` 「종료 / 결과」) **몰라서 틀린 것과 못 푼 것은 다음에 할 일이
다르므로** 화면에서 가른다. (2026-09-08, SJO-24)
...
| `/exam/[id]/result/[N]` | 이전 · **다음** — 65칸 그리드가 요약 화면에 있어 여기는 이웃 문항으로만 옮긴다 |
| `/exam/[id]/result` | 없다 — 읽는 화면이고 나가는 길은 앱바 뒤로가기다 |
...
**「숨기지 않는다」가 걸리는 것은 목록의 양 끝이다.** 첫 문제의 「이전」이 원래의 그 자리이고,
`/exam/[id]/result/[N]`에서는 마지막(65)의 「다음」도 같다 — 그 화면은 65문항을 오가며 **읽는**
화면이라 「다음」이 진행이 아니라 이동이고, 끝에서 사라지면 「그런 기능이 없다」로 읽힌다. 근거는
같으므로 규칙을 늘리지 않고 자리를 하나 더 세는 것으로 둔다 (2026-09-08, SJO-24).

**「종료」에 `error`를 쓰지 않는다.** 파기가 아니라 채점으로 넘어가는 정상 경로이고, 되돌릴 수 없는
쪽(진행 중 세션 포기)은 확인 다이얼로그가 받는다 (`docs/02-features.md` 「세션 생성」).

### 뒤로가기는 히스토리가 아니라 라우트가 정하는 고정 링크다
...
화살표도 두지 않는다. 기본 펼침인 카드는 내용이 이미 보이고, 접힌 카드는 카드 전체가 클릭 영역이다.

**펼칠 것이 없는 카드는 `details`가 아니다.** 오답 해설은 오답 선택지 전체를 정확히 덮으므로
(실측 1019/1019) 오답 카드는 언제나 눌리지만, 정답 카드는 **첫 하나만** 해설을 갖는다
(「② 정답 해설」). 정답이 둘 이상인 123문항에서 나머지 정답 카드가 그 경우다.

### 결과 배너

**지문 바로 위** 한 줄이다. `/study`·`/review`에서는 화면의 첫 블록이 아니다 — 저장 대기 배너와 적용된 필터 칩이 위에 올 수 있다. 결과의 문항 리뷰(`/exam/[id]/result/[N]`)에는 그 둘이 없어 이 배너가 첫 블록이다. 순서의 정본은 `docs/02-features.md` 「화면 구성 요소」다.
...
**제목 「요구사항」을 `title-small`로 단다.** ②④도 같다 — 「정답 해설」·「등장 서비스」.
**③에는 제목을 달지 않는다** — 아래 「③ 오답 해설」이 *헤더는 선택지 카드 자체다*로 정한 것이
그것이고, 카드 안에 「오답 해설」을 또 적으면 그 카드에서만 제목이 둘이 된다 (2026-09-07, SJO-20).
...
**정답 선택지가 둘 이상이면 첫 정답 카드에만 넣는다.** 정답 해설은 문항당 한 덩어리인데
(`Question.explanation`) 정답 선택지는 2~3개일 수 있다 — 실측 **123문항**(2개 109 · 3개 14)이
그렇다. 카드마다 복제하면 같은 본문이 화면에 두세 번 뜨는데, 그건 ③에서 아코디언을 쓴 이유
(화면 길이)와 정면으로 어긋난다. 나머지 정답 카드는 **펼칠 것이 없으므로 `details`가 아니라
정적 카드**다 — 면색·테두리·아이콘은 위 표 그대로다. 정답이 하나인 896문항(88%)에서는 아무
차이가 없다 (2026-09-07, SJO-20).
...
**이 링크는 아직 그리지 않는다 — 매핑 데이터가 없다.** `oneliners.json`의 `OneLiner`는
`service`·`category`·`note` 셋뿐이고(`04-data-model.md`), 해부서 목차는 절 제목과 쪽 번호만
갖는다. 서비스↔절을 잇는 것이 없어 `/study`는 칩과 한줄노트까지만 그린다. 매핑을 만드는 것은
`/anatomy`가 들어오는 E7의 몫이다 (2026-09-07, SJO-20).
...
| 보조 버튼 | **`surface-container` 면 + `primary` 글자** · `corner-full` · 높이 56px |
...
**보조 버튼에 면을 주는 이유가 이것이다.** `compact`에서 스크롤 중인 지문은 버튼 뒤로 지나간다.
`primary` 채움인 주 버튼은 버티지만 **면색 없는 글자는 겹치면 버튼도 본문도 읽을 수 없다** —
2026-09-07 `/study` 실화면에서 「이전」이 선택지 E 위에 겹쳐 재현됐다 (SJO-20).

`surface-container`를 고른 이유는 둘이다. **바 전체에 배경을 되돌리지 않는다** — 그러면 위
문단이 말한 「바가 아니라 버튼만 뜬다」가 무너지고, 화면 하단 88px이 상시 면으로 잠긴다.
그리고 새 색 역할을 만들지 않는다 — `surface-container`는 이미 요구사항 블록과 진행 중
모의고사 배너가 쓰는 「본문 위에 얹히는 면」이다 (「surface container를 화면에 매핑」).

**이 면은 하단 액션 바의 보조 버튼에만 준다.** 상태 배너의 「다시 시도」는 이미
`surface-container` 배너 안에 있어 같은 면색을 주면 경계가 사라진다 (「상태 배너」).

**면 위의 `primary` 글자가 「색 사용 규칙」과 어긋나지 않는 이유.** 그 규칙은 `primary`를
링크와 주 버튼에 묶어 두는데, 이 버튼은 둘 다 아니다. 그래도 성립하는 것은 **한 바에 주 버튼과
짝으로만 서기 때문이다** — `primary` 채움(주)과 `primary` 글자(보조)가 나란히 있어 「이 둘이
이 화면의 행동」이 한 덩어리로 읽히고, 선택 상태(`secondary-container`)와 경쟁하지 않는다.
`primary`가 정오를 나르지 않는다는 잠금도 그대로다.
...
대시보드가 소유하는 여덟이다. 나머지 화면은 위 컴포넌트로 조립되고, **전체 진도 카드와 카테고리별
정답률 막대 둘은 결과 화면이 같은 규격으로 재사용한다** (각 행에 그 차이를 적었다).

| 요소 | 규격 |
|---|---|
| 전체 진도 카드 | `surface-container-low` · **`outline` 1px** · `corner-medium` · 패딩 16px. 「전체 진도」 `body-small` → `247 / 1019` (`headline-small`, 분모는 `body-large`·`on-surface-variant`) → 8px 트랙. **결과 화면의 점수 카드는 같은 규격에서 트랙만 뺀다** — 점수는 진행이 아니라 결과라 「남은 양」이 없다 (2026-09-08, SJO-24) |
| 이어풀기 | 주 버튼 규격 그대로 (`primary` · `corner-full` · 56px). 폭은 전체. `/study`로 간다 |
| 진행 중 모의고사 이어풀기 | `surface-container` · **`outline` 1px** · `corner-medium` · 최소 48px · 문구 `body-medium` + 우측 텍스트 버튼. **상태 배너가 아니다** — 그쪽은 로딩·오류·저장 대기 셋 전용이다. **`/exam` 목록이 같은 규격을 쓴다** — 거기서는 텍스트 버튼이 둘(「이어풀기」·「포기」)이다. 포기 경로가 대시보드에는 없다 (`docs/02` 「목록 화면」) |
| 카테고리별 정답률 막대 | `title-small` 제목 + 막대 목록. 한 줄은 라벨(`body-small`·88px) · 8px 트랙(`surface-container-high`) · 값(`body-small`·우측 정렬). 줄 높이 36px — 누를 수 없으므로 탭 타겟이 아니다. **값의 형식은 화면이 정한다** — 대시보드는 백분율(`62%`), 결과 화면은 `정답 / 문항`(`4 / 6`)이다. 한 세션의 카테고리당 문항이 2~15개라 백분율이 표본 크기를 지운다. 색 경계는 둘 다 60%다 |
...
**경계선이 진행 중 카드와 상태 배너를 가른다.** 둘은 면·shape·타입스케일·최소 높이가 같아서
(`surface-container` · `corner-medium` · `body-medium` · 48px) 나란히 서면 아이콘 하나로만 갈리는데,
`/exam` 목록에서는 실제로 나란히 선다 — 409 직후 「진행 중인 모의고사가 있다」 배너 바로 아래가 그
카드다. 카드에만 `outline` 1px을 준다. 「본문 안에 놓이는 면에는 경계를 준다」는 「화면 보조 패널」의
선례와 같은 논리다 (2026-09-08 실화면 리뷰, SJO-23).
...
| 옵션 컨트롤 | **액션 앞에 선다.** 액션이 아니라 *액션이 무엇을 할지 정하는 것*이라 자리가 다르다 — `/exam` 빈 상태의 「안 푼 문제 우선」 칩이 유일한 경우다. 액션 슬롯에 섞으면 「주 버튼 하나」가 무엇이든 받는 자루가 된다 (2026-09-08, SJO-23) |
| 간격 | 문구 ↔ 액션 16px |
...
키 컬러는 **결정됐다** — scent-jo 시그니처 주황 `#E68236`에서 재생성한 팔레트를 쓴다 (「팔레트」). 시그니처 그라데이션(`#E68236 → #F7A863`)은 아직 도입하지 않는다. 블로그는 로고타입·GNB 강조 링크·CTA 배너에 쓰는데 이 앱 화면 10개엔 셋 다 없다 — 칠할 표면이 생기면(= 위 표의 로고를 결정하면) 그때 연다.
