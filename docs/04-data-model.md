...
**서버(`catalog`)는 반대로 manifest를 다시 확인한다 — 요청이 있을 때 최대 5분에 한 번이다** (`05-database.md` 「catalog 모듈」). 타이머로 도는 폴링이 아니다. 비대칭이 의도된 것이다 — 서버는 낡은 정답으로 채점하면 안 되고, 클라이언트는 세션 중간에 데이터가 갈리면 안 된다.
...
판단 근거는 하나다 — **원문 전체에 반례가 있는가**. 하이픈 50자리·가운뎃점 9자리·붙은 슬래시 15자리는 원문에 띄운 형태가 단 한 건도 없어 규칙으로 만들었다. 반례가 있는 부류(`한글`+`(`는 79자리 중 5자리가 진짜 삽입구)는 규칙으로 만들지 않고 오라클로 넘긴다 (SJO-42).

**조사는 반례가 있었다.** 부사 「보다」가 조사 「보다」와 겹쳐, 문장이 끝난 자리에서 다음 문장의 부사를 앞 문장에 붙였다(1019 중 1문항). `이`처럼 목록에서 빼는 대신 **왼쪽이 `.`·`?`·`!`로 끝나면 조사 규칙을 적용하지 않는 것**으로 갈랐다 — 조사는 문장이 끝난 자리에 올 수 없다. 코퍼스의 조사 이음매 1090자리 중 문장부호 뒤는 그 1자리뿐이다 (2026-09-01 전수, SJO-7 리뷰).
