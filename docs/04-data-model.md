...
같은 이유로 **앞 글자에 붙은 슬래시**(`내구성/`+`고가용성`)와 **다음 줄이 조사로 시작하는 자리**(`Amazon EC2`+`를`)도 붙인다. 조사는 앞말과 띄어 쓰지 않는다. 다만 **`이`는 뺐다** — 코퍼스 10자리 중 셋이 "이 요구사항"류의 지시관형사라 붙이면 오히려 틀린다. 띄어 쓴 슬래시(`신호 하나 /`)는 구분자라 그대로 둔다.
