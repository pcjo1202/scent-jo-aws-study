...
**`categories`가 빌 수 있다.** 노트에 없는 개념(MFA·SAML·STS 등)만 언급하는 문항이 1019 중 6개 있다. 카테고리는 매칭된 서비스에서만 나오므로 그런 문항은 `services`·`categories`가 함께 빈다 — 없는 카테고리를 지어내지 않는다 (2026-08-28 실측, SJO-6).
...
**측정 범위는 한글끼리 만나는 이음매다.** 한쪽이라도 라틴·숫자면 줄바꿈을 단어 경계로 보고 띄우는데, 그 전제가 **줄이 하이픈으로 끝날 때** 깨진다 — `ap-`+`northeast-2`, `S3 Standard-`+`Infrequent Access`처럼 하이픈이 토큰 안쪽이기 때문이다. 코퍼스의 하이픈 줄바꿈 50자리가 전부 이 형태여서 **줄 끝 하이픈은 무조건 붙인다** (2026-08-31 전수, SJO-7).

같은 이유로 **앞 글자에 붙은 슬래시**(`내구성/`+`고가용성`), **가운뎃점**(`읽기·`+`쓰기`), **다음 줄이 조사로 시작하는 자리**(`Amazon EC2`+`를`)도 붙인다. 조사는 앞말과 띄어 쓰지 않는다. 다만 **`이`는 뺐다** — 코퍼스 10자리 중 셋이 "이 요구사항"류의 지시관형사라 붙이면 오히려 틀린다. 띄어 쓴 슬래시(`신호 하나 /`)는 구분자라 그대로 둔다.

네 부류의 판단 근거는 같다 — **원문 전체에 반례가 없다**. 하이픈 50자리·가운뎃점 9자리는 원문에 띄운 형태가 단 한 건도 없고, 슬래시·조사도 마찬가지다. 반례가 있는 부류(`한글`+`(`는 79자리 중 5자리가 진짜 삽입구)는 규칙으로 만들지 않고 오라클로 넘긴다 (SJO-42).

수정 후 파서가 만든 어절 분리는 **0건**이다. 산출물에 남은 30자리는 원문의 줄 **안쪽**에 그대로 있는 형태여서 그대로 옮긴 것이다 — 원문을 고쳐 쓰지 않는다.

**골든 픽스처는 이 부류를 원리적으로 못 잡는다** — `08-testing.md`가 "비교는 공백을 지우고 한다"로 정해 뒀기 때문이다. 구조 검증(`data:verify`)도 못 잡는다. 내용 정확도 검수가 유일한 방어선이었다.
...
100문항 단위, 파일당 **56~272KB**(들여쓰기 없는 JSON, 2026-08-31 실측). 마지막 청크(011)만 19문항이다.

크기는 문항 길이가 정하고 gzip이 3분의 1로 줄인다. **청크 파일은 CDN에 그대로 올라가므로 들여쓰기를 넣지 않는다** — 사람이 여는 파일이 아니다.
...
1019개, **141KB**(2026-08-31 실측 — `services`의 서비스명과 한글 카테고리가 대부분이다). **필터·목록 렌더링을 전부 이 파일만으로 처리한다.** 모의고사 추첨은 서버 `catalog`가 한다 (`05-database.md`). 실제로 문제를 풀 때만 해당 청크를 받는다.
...
노트는 S3·IAM·FSx 같은 서비스를 **기능 단위로만** 싣는다 (`S3 Standard`는 있고 `Amazon S3`는 없다). 지문은 맨 이름을 훨씬 자주 쓴다 — 맨 `S3`만 289문항에 나온다.

보완을 빼고 돌리면 미태깅이 6 → 63문항으로 늘고 **스토리지가 355 → 151문항(34.8% → 14.8%)으로 무너진다.** 맨 `S3`를 말하는 289문항 중 222문항이 스토리지 태그를 잃고, 그중 40문항은 아무 서비스에도 걸리지 않는다.

루트 이름 8개(`S3`·`IAM`·`FSx`·`Kinesis`·`ElastiCache`·`Storage Gateway`·`Snowball`·`Shield`)를 보완하되 **카테고리는 적지 않는다.** 그 루트로 시작하는 **별칭을 가진 서비스**의 다수결로 끌어온다 (한 서비스가 별칭을 여럿 내도 한 표다. `AWS Shield Standard`는 이름이 `Shield`로 시작하지 않지만 벤더 접두사를 뗀 별칭이 시작하므로 참여한다) — `S3 Bucket Policy` 하나가 보안이어도 나머지 18개가 스토리지라 루트는 스토리지가 된다.
...
- **분포를 검증한다.** `data:extract`가 카테고리 분포와 미태깅 문항 수를 출력하고, **한 카테고리가 전체 문항의 50%를 넘거나 미태깅이 10%를 넘으면 산출을 막는다** (2026-08-28 결정, SJO-6)

**미태깅 상한이 따로 필요한 이유**: 사전이 통째로 무너지면 아무 카테고리도 붙지 않아 편중 상한이 발동하지 않는다. 두 상한이 서로 다른 붕괴를 막는다 — 편중은 "한쪽으로 쏠렸다", 미태깅은 "아무것도 안 붙었다". 실측 0.6%이고, 루트 별칭을 통째로 빼도 6.2%에 그친다. 10%는 그 위의 붕괴만 잡는다.
...
`data/`는 **평면**이고 CDN은 `questions/`·`notes/`로 갈린다 (`03-architecture.md` 「경로 레이아웃」). **그 대응표가 manifest의 키다** — publish는 키를 보고 올리고 클라이언트는 `base` + 키로 받는다. 대응표를 따로 두지 않는다.

`data:verify`는 **원본 PDF를 보지 않는다.** `data/`와 `tests/fixtures/`만 읽어 판정하므로 `data:pull`로 받아 온 기기에서도 그대로 배포 게이트 구실을 한다.
...
`base`(랜덤 프리픽스를 포함한 실제 CDN 경로)와 버전은 `DATA_CDN_BASE`·`DATA_VERSION` 환경변수로만 들어온다 — 레포에 커밋하지 않는다. 없으면 `base`가 빈 문자열이고 버전은 `v1`이다.
...
**표본은 층별 최소 1문항 + 희귀 형태 보장 후 비례 배분이다** (2026-08-31 결정, SJO-7). 순수 비례로 뽑으면 3개 정답(14문항)·선택지 6개(14문항)·코드 블록(7문항)이 표본에서 사라지는데, 파서가 깨지는 자리가 바로 거기다. 소수 층(운영·AI/ML·마이그레이션·미태깅)도 같은 이유로 0이 된다. 추첨은 시드를 고정해 같은 데이터에서 같은 표본이 나오게 하고, 뽑힌 문항 번호를 이슈 코멘트에 남긴다.
