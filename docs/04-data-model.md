...
**측정 범위는 한글끼리 만나는 이음매다.** 한쪽이라도 라틴·숫자면 줄바꿈을 단어 경계로 보고 띄우는데, 그 전제가 **줄이 하이픈으로 끝날 때** 깨진다 — `ap-`+`northeast-2`, `S3 Standard-`+`Infrequent Access`처럼 하이픈이 토큰 안쪽이기 때문이다. 코퍼스의 하이픈 줄바꿈 50자리가 전부 이 형태여서 **줄 끝 하이픈은 무조건 붙인다** (2026-08-31 전수, SJO-7). 골든 픽스처는 공백을 지우고 비교하므로 이 부류를 못 잡는다 — 내용 정확도 검수가 잡아낸 결함이다.
