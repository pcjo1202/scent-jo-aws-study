...
**셀렉터를 `className` 안으로 좁히지 않는다.** 좁히면 클래스 문자열을 상수로 뽑는 순간 규칙이 꺼지는데, 상태별 클래스를 레코드로 두는 것(`const RESULT_STYLE = { … }`)은 컴포넌트에서 기본형이다. 우회 경로 6가지를 한 파일에 담아 세었더니 좁힌 셀렉터는 4건만 잡았다 (2026-08-28, SJO-18). 대신 유틸 모양(`<접두사>-[값]`)을 요구해 배열 인덱싱·정규식 문자열을 오탐하지 않게 한다.
