...
색각 쪽에는 대응하는 기지값이 없어 **위생 검사**로 받는다. 다만 이슈가 요구한 「시뮬레이션 ΔE ≤ 정상 ΔE」는 **CIE76에서 엄밀하게 참이 아니다.** 이색각 투영은 LMS에서 거리를 줄이지만 Lab은 다른 계량이라, 올바른 선형공간 구현에서도 126건(42쌍 × 3모델) 중 **13건이 정상보다 크다.**

그래서 초과분에 문턱을 둔다. 문턱은 JND(2.3)다 — 식별 한계보다 작은 증가는 애초에 보이지 않으므로 버그의 증거가 아니다. 그리고 **비율이 아니라 절대 ΔE로 잰다.** 실측한 분리력이 그렇게 말한다.

| | 최대 초과 (절대 ΔE) | 최대 초과 (비율) |
|---|---|---|
| 선형 공간 (정본) | **+1.93** | 1.084 |
| 감마 공간 (SJO-38의 버그) | **+6.19** | 1.112 |

절대값은 3.2배 벌어지는데 비율은 2.6%만 움직인다 — 정상 ΔE가 작은 쌍에서 비율이 잡음을 증폭하기 때문이다. 문턱 2.3은 정본을 통과시키고 버그를 잡는다(음성 대조 확보). 여유가 넓지 않으므로(1.93 vs 2.30) **관측된 최대 초과폭을 매 실행 출력한다** — 팔레트가 바뀌어 문턱에 다가가면 터지기 전에 보인다.

#### 표 밖 산문 수치도 대조한다

같은 값이 표에만 있지 않다. 「42쌍 중 JND 미만이 N건」처럼 표에서 파생된 **집계**, 「적록색약 ΔE N」처럼 다른 절이 인용한 **상수**가 남는다. 표만 고치고 산문을 두면 **둘 다 정본으로 읽혀 공백보다 위험하다**(루트 `CLAUDE.md`). 그래서 산문 주장도 정규식으로 뽑아 계산값과 대조하고, **패턴마다 모든 출현을 센다** — 첫 일치만 보면 같은 값이 다른 절에서 갈라진 것을 놓친다(실제로 그렇게 한 번 놓쳤다).

주장을 **못 찾는 것도 실패**다. 문장을 고쳐 쓰다 정규식이 빗나가면 검사가 조용히 0건이 되는데, `exit 0`은 "통과"가 아니라 "오류 없음"이다.

#### 우회 경로를 프로브로 셌다

값을 일부러 어긋나게 만든 사본을 돌려 검출되는지 셌다 (2026-09-08, SJO-39). 이슈가 지정한 셋에 **0건 실행**과 **주장 소실** 둘을 더했다 — `exit 0`이 "대상 없음"으로 새는 자리가 그 둘이다.

| 프로브 | 기대 | 결과 |
|---|---|---|
| 손대지 않은 사본 | 통과 | **통과** (오탐 0건) |
| `--ref-primary-40` hex 1자리 변조 | 실패 | **실패** — 팔레트 1건 + 색각 2건 |
| 「대비 검증」 표 숫자 1개 변조 (16.37 → 16.38) | 실패 | **실패** — 대비 1건 |
| 색각 계산을 감마 공간으로 되돌림 | 실패 | **실패** — 색각 표 8건 + 위생 1건 |
| 「대비 검증」 제목을 바꿔 표를 못 찾게 함 | 실패 | **실패** — 파서가 던진다 |
| 산문 주장 문장을 바꿔 정규식이 빗나감 | 실패 | **실패** — 산문 2건, 검사 10 → 8건 |

**6개 중 6개가 기대와 일치했다.** 첫 줄이 음성 대조다 — 이것을 빼먹으면 하네스가 죽었을 때 「실패해야 하는」 다섯 개가 전부 공짜로 통과한다. 실제로 첫 실행에서 프로브 디렉터리 이름이 어긋나 여섯 개 전부가 모듈 해석 오류로 죽었고, **그 다섯은 「검출됨」으로 집계됐다.** 음성 대조 한 줄이 그것을 잡았다.

**가드가 둘이면 따로 지워 본다.** 감마 회귀는 「색각 표 대조」와 「위생 검사」 둘 다에 걸린다. 표 대조를 무력화한 사본에서 위생 검사가 **혼자서도** 잡는지 확인했다(최대 초과 +6.19 > 문턱 2.30). 앞의 가드만 보고 「확인함」으로 세면 뒤의 방어선을 검증하지 않은 채 넘어간다.
