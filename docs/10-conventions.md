...
그래서 훅(커밋 시점)과 `/done` 1단계의 공통 게이트(이슈 종료 시점)를 **둘 다** 건다. 훅에 남는 구멍이 정확히 둘이고, 그 둘을 `/done`이 덮는다.

- `git commit --no-verify` — git이 주는 탈출구라 막을 수 없다
- `pnpm install`을 한 번도 안 돌린 clone — `core.hooksPath`가 안 걸려 훅이 **조용히** 건너뛰어진다

우회 경로 7개를 실제 커밋으로 재서 7개 전부가 기대와 일치했다 (막힘 2 · 통과 5, 오탐 0). 훅을 끈 경로에서 같은 파일이 통과하는 것이 음성 대조다 — 「막힘」이 훅 때문임을 그것이 보인다.
...
**`prepare`에 `|| true`를 붙인다.** `.git`이 없는 환경에서 `git config`가 실패하면 `pnpm install`이 통째로 죽는다. Vercel은 소스를 tarball로 받으므로 가드가 없으면 배포가 깨진다.
