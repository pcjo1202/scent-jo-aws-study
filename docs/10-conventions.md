...
화면 10개에 FSD는 과할 수 있다. 공식 문서도 *"대부분의 프로젝트는 Shared·Pages·App만 있으면 된다"*, 별도 가이드는 *"20개 이상 기능으로 성장할 앱에 권장"* 이라고 한다. 그럼에도 쓰는 이유는 **에이전트가 코드를 쓰기 때문**이다 — 배치가 예측 가능하고 임포트 방향이 린터로 강제되면, 세션이 바뀌어도 같은 자리에 같은 것이 놓인다.
...
`useQuery` + `isLoading`/`isError` 분기는 화면 10개에 그대로 10번 복제된다. `useSuspenseQuery`는 `data`가 항상 있는 타입을 주므로 **분기 자체가 사라지고**, 로딩·오류 표현이 경계 컴포넌트 한 곳으로 모인다. `docs/02`의 「정적 데이터(CDN) 실패」가 요구하는 "오류 + 재시도"도 그 한 곳에서 구현된다.
...
**셀렉터를 `className` 안으로 좁히지 않는다.** 좁히면 클래스 문자열을 상수로 뽑는 순간 규칙이 꺼지는데, 상태별 클래스를 레코드로 두는 것(`const RESULT_STYLE = { … }`)은 컴포넌트에서 기본형이다. 대신 유틸 모양(`[-]<접두사>-[값]`)을 요구해 배열 인덱싱·정규식 문자열을 오탐하지 않게 한다.

우회 경로를 프로브 파일 하나에 담아 센 결과다 (2026-08-28, SJO-18). 정상 케이스 3건(`data-[…]:` · `[&>svg]:` · 토큰 클래스)은 세 차례 모두 오탐 0건.

| 셀렉터 | 검출 |
|---|---|
| `className` 하위로 좁힌 원안 | 6건 중 **4건** — `const X = 'gap-[7px]'`와 `{ a: 'mt-[3px]' }`를 놓친다 |
| 파일 전역 + 유틸 모양 | 6건 중 6건 |
| 〃 + 선행 하이픈 허용 | **7건 중 7건** — 음수 유틸(`-mt-[3px]`)이 그 전까지 새고 있었다 |

**세 번째 줄이 이 표의 요점이다.** "6건 중 6건"은 목록에 없는 경로를 보증하지 않는다. 프로브를 늘리는 것이 규칙을 고치는 것보다 먼저다.
...
### 토큰 검증 — 왜 `pnpm tokens:verify`인가

`DESIGN.md`가 발행하는 색 수치(팔레트 hex·대비비·색각 ΔE)는 **`tokens.css`에서 계산해 나온 값**이다. 계산이 문서 밖에서 한 번 일어나고 값만 옮겨 적히면 그 값이 맞는지 다시 볼 방법이 없어진다. SJO-38이 정확히 그렇게 했고, 색각 시뮬레이션을 감마 공간에서 계산한 탓에 **정상 시각보다 ΔE가 큰 불가능한 값**이 문서·PR·이슈 코멘트에 실렸다. 리뷰어의 독립 재계산이 아니었으면 남았다.

그래서 계산을 레포 안으로 들여 매번 다시 돌린다. `scripts/src/tokens/`가 `tokens.css`를 파싱해(`--sys-color-*` → `--ref-*` → hex) 문서의 세 표와 표 밖 산문 수치를 전부 대조하고, 어긋나면 non-zero exit이다. `data:verify`와 같은 모양이다 — 판정은 순수함수(`verify-tokens.ts`)가 하고 읽기·출력은 엔트리가 한다.

#### 검증만 넣고 생성은 넣지 않는다

팔레트 **생성**(`CorePalette.of` 등)은 스크립트에 없다. 명령은 `tokens.css` 머리주석과 `DESIGN.md` 「팔레트」에 글로만 남긴다.

| | 왜 |
|---|---|
| 도는 빈도가 다르다 | 생성은 브랜드 키 컬러를 갈 때만 돌고, 검증은 커밋마다 돈다. 한 명령에 묶으면 매번 안 쓰는 절반을 끌고 다닌다 |
| 의존성 비용이 한쪽에만 있다 | 생성은 `@material/material-color-utilities`를 설치해야 하는데 지금 이 레포에 없다. 검증은 의존성이 0이다 — 표준 색공간 변환과 공개된 행렬뿐이라 직접 쓰는 편이 짧다 |
| 생성기를 넣어도 판정은 검증기가 한다 | 생성 결과가 맞는지는 결국 이 검증기가 본다. 생성기는 값을 만들 뿐 지켜 주지 않는다 |

되돌릴 조건은 분명하다 — 팔레트를 **두 번 이상** 갈게 되면 그때 생성도 스크립트로 내린다.

#### 계산기를 먼저 검증한다

수치를 발행하기 전에 그 수치를 만든 계산기를 검증한다. 매 실행마다 기지값 셋을 재현시키고, 하나라도 어긋나면 나머지 검사를 신뢰하지 않는다.

| 기지값 | 무엇을 지키나 |
|---|---|
| `#6750A4` + 흰 = **6.44** · `#B3261E` + 흰 = **6.54** | WCAG 상대휘도와 대비비. M3 baseline의 알려진 값이라 이 레포 밖에서 왔다 |
| `Lab(#FF0000)` = **53.24 / 80.09 / 67.20** | sRGB → 선형 → XYZ → Lab 경로 전체. ΔE가 이 위에 얹힌다 |

**이 셋은 색각 경로를 덮지 않는다.** 시뮬레이션에는 대응하는 기지값이 없어 아래 위생 검사가 유일한 음성 대조다. 그 절반이 검증에서 약한 쪽이라는 것을 알고 써야 한다 — 실제로 이 이슈에서 값을 세 개 잘못 옮긴 자리가 정확히 거기였다.

#### 시뮬레이션은 sRGB 게멋으로 자른다

`simulate()`는 행렬을 곱한 뒤 **[0, 1]로 클램프한다.** 이색각 행렬은 색역 밖 값을 내놓는다 — `primary` `#964900`은 Viénot deutan에서 파랑이 **-0.0053**이 된다. 표시할 수 없는 색이고, 0 근처는 sRGB 전달함수가 가팔라 그 음수 하나가 Lab b\*의 차이를 통째로 만든다. 화면은 그 채널을 0으로 자르므로 **이색각 사용자가 실제로 보는 차이는 자른 쪽**이다.

| `primary` / `error` (Light) 색각 최악 | |
|---|---|
| 클램프 (정본) | **2.0** — JND 2.3 미만 |
| 클램프 없음 | 5.6 — JND 통과 |

이 결정은 사후에 붙인 것이 아니다. **SJO-39가 클램프 없이 만들고, 그 값으로 「JND 미만 2건」을 1건으로 고쳐 발행했다.** 접근성 결론을 관대한 쪽으로 민 뒤 리뷰에서 되돌렸다. 원인은 하나다 — 옛 값이 어떤 파이프라인에서 나왔는지 **반례를 찾지 않고** 「어떤 모델로도 재현되지 않는다」고 단정했다. 클램프를 넣으면 옛 표 8행이 **8/8 재현된다.** 루트 `CLAUDE.md` 「관측 몇 건으로 세운 가설은 수정을 적용하기 전에 반례를 먼저 찾는다」가 그대로 걸린 자리다.

**표시할 수 없는 색이 만든 ΔE를 발행하지 않는다** — 그것이 이 절의 규칙이다.

#### 위생 검사는 허용오차로 받는다

이슈가 요구한 「시뮬레이션 ΔE ≤ 정상 ΔE」는 **CIE76에서 엄밀하게 참이 아니다.** 이색각 투영은 LMS에서 거리를 줄이지만 Lab은 다른 계량이라, 올바른 구현에서도 126건(42쌍 × 3모델) 중 일부가 정상보다 크다.

그래서 초과분에 문턱을 둔다. 문턱은 JND(2.3)다 — 식별 한계보다 작은 증가는 애초에 보이지 않으므로 버그의 증거가 아니다. 그리고 **비율이 아니라 절대 ΔE로 잰다.**

| | 최대 초과 (절대 ΔE) | 판정 |
|---|---|---|
| 선형 + 클램프 (정본) | **+1.61** | 통과 |
| 감마 공간 (SJO-38의 버그) | **+6.19** | 검출 |

비율로 재면 같은 두 경우가 1.06 대 1.11로 붙어 가른다고 보기 어렵다 — 정상 ΔE가 작은 쌍에서 비율이 잡음을 증폭하기 때문이다. 여유가 넓지 않으므로(1.61 vs 2.30) **관측된 최대 초과폭을 매 실행 출력한다.**

**이 검사가 가르지 못하는 것도 적어 둔다.** 클램프 유무는 최대 초과가 1.61 대 1.93으로 **둘 다 문턱을 통과한다.** 위 게멋 결정이 위생 검사로 강제되지 않는다는 뜻이고, 그래서 규칙으로 문서에 남긴다.

#### 검사 대상이 줄어드는 것을 검사한다

값 대조는 표에 있는 행만 본다. 행이 빠지거나 역할이 늘면 대조 건수만 조용히 줄고 `exit 0`이 나온다 — 루트 `CLAUDE.md`가 "0건보다 위험하다"고 적은 부분 집계다. 그래서 셋을 따로 센다.

| 완비성 검사 | 막는 것 |
|---|---|
| `--sys-color-*` 역할이 전부 유채·무채로 분류돼 있는가 | 새 역할이 위생 검사에서 조용히 빠지는 것 |
| 「대비 검증」이 모든 `on-` 역할을 한 번 이상 재는가 | 표에서 행이 사라지는 것 |
| 「색으로만 갈리지 않는 쌍」이 선정 기준(최악 4쌍 + `error`×`correct` 계열)대로인가 | 값이 바뀌었는데 표가 더 이상 최악을 안 보여주는 것 |

표 파서는 **머리글을 글자로 확인한다.** 첫 `|` 행을 머리글이라고 가정하면 머리글이 지워졌을 때 본문 한 줄이 대신 버려진다.

#### 표 밖 산문 수치도 대조한다

같은 값이 표에만 있지 않다. 「42쌍 중 JND 미만이 N건」처럼 표에서 파생된 **집계**, 「적록색약 ΔE N」처럼 다른 절이 인용한 **상수**가 남는다. 표만 고치고 산문을 두면 **둘 다 정본으로 읽혀 공백보다 위험하다**(루트 `CLAUDE.md`).

그래서 산문 주장도 정규식으로 뽑아 계산값과 대조하되, 두 가지를 함께 건다.

- **패턴마다 모든 출현을 센다.** 첫 일치만 보면 같은 값이 다른 절에서 갈라진 것을 놓친다.
- **출현 횟수를 박는다.** 모든 출현을 세는 것으로는 **패턴이 표기 하나에만 맞는 경우**를 못 막는다. 한국어는 조사·어순이 갈려(「`surface-container-low`**가**」 vs 「**는**」) 좁은 정규식이 한 곳만 짚고 나머지 둘을 통과시킨다 — 실제로 1.05가 세 곳 중 한 곳만 검증되고 있었다. 값 쪽으로 느슨하게 잡고 기대 출현 수를 함께 적는다.

주장을 **못 찾는 것도 실패**다. 문장을 고쳐 쓰다 정규식이 빗나가면 검사가 조용히 0건이 되는데, `exit 0`은 "통과"가 아니라 "오류 없음"이다.

#### 우회 경로를 프로브로 셌다

값을 일부러 어긋나게 만든 사본을 돌려 검출되는지 셌다 (2026-09-08, SJO-39). 이슈가 지정한 셋으로 시작해, 리뷰가 뚫은 자리를 넣어 열하나가 됐다.

| 프로브 | 결과 | 무엇이 잡았나 |
|---|---|---|
| 손대지 않은 사본 | **통과** | — (오탐 0건) |
| `--ref-primary-40` hex 1자리 변조 | 검출 | 팔레트·색각 |
| 「대비 검증」 숫자 1개 변조 | 검출 | 대비 |
| 색각 계산을 감마 공간으로 되돌림 | 검출 | 색각·위생 |
| 「대비 검증」 제목 변경 | 검출 | 파서가 던진다 |
| 산문 주장 문장 변경 | 검출 | 산문(주장 소실) |
| 「대비 검증」 **머리글 행** 삭제 | 검출 | 파서가 던진다 |
| 「대비 검증」 **본문 행** 삭제 | 검출 | 완비성 |
| 「색으로만…」 **본문 행** 삭제 | 검출 | 완비성 |
| **게멋 클램프 제거** | 검출 | 색각 |
| 새 유채 역할 추가(분류 누락) | 검출 | 완비성 |

**11개 중 11개가 기대와 일치했다.** 첫 줄이 음성 대조다 — 이것을 빼먹으면 하네스가 죽었을 때 「실패해야 하는」 나머지가 전부 공짜로 통과한다. 실제로 첫 실행에서 프로브 디렉터리 이름이 어긋나 전부가 모듈 해석 오류로 죽었고, **그것들이 「검출됨」으로 집계됐다.** 음성 대조 한 줄이 잡았다.

**뒤의 다섯은 처음에 없었고, 없는 동안 실제로 뚫려 있었다.** 앞의 여섯만 세고 「6개 중 6개」로 발행했는데, 리뷰가 본문 행 삭제·머리글 삭제·클램프 제거를 각각 통과시켰다. 프로브 개수는 안심의 근거가 아니다 — **세어본 적 없는 경로가 사각지대**이고, 그래서 열거를 여기 남긴다.

**가드가 둘이면 따로 지워 본다.** 감마 회귀는 「색각 표 대조」와 「위생 검사」 둘 다에 걸린다. 표 대조를 무력화한 사본에서 위생 검사가 **혼자서도** 잡는지 확인했다. 앞의 가드만 보고 「확인함」으로 세면 뒤의 방어선을 검증하지 않은 채 넘어간다.

파서 프로브는 `scripts/src/tokens/*.spec.ts`에 스펙으로도 남겼다 — 프로브는 한 번 돌고 끝나지만 스펙은 `pnpm test`가 매번 돌린다.

#### 게이트는 훅과 `/done` 둘 다

`format:check`와 같은 자리에 얹는다 — 이유도 같다. `pre-commit`은 `git commit` 경로에만 붙어 머지 커밋·rebase·`--no-verify`가 통째로 새고, **이 레포의 유일한 머지 전략인 merge commit은 GitHub 서버에서 일어나 훅이 존재하지도 않는다.** 훅은 빠른 되먹임이고 방어선은 `/done` 1단계다.
