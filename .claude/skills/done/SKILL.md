...
3. **리뷰** — 그 diff로 `pr-review-toolkit:code-reviewer` 서브에이전트 실행. `apps/web`의 컴포넌트·스타일 변경이 포함되면 `design-reviewer`도 실행. P1은 수정 필수, P2는 수정하거나 보류 사유를 코멘트로 남긴다.
4. **재검증** — 리뷰로 코드를 고쳤으면 **DoD를 다시 실행한다.** 리뷰 반영이 DoD를 깨는 일은 흔하다. 수정분은 커밋.
5. **Linear 갱신** — 체크박스를 patch로 `[x]` → 검증 증거(명령+결과 요약, 리뷰 결과 한 줄)를 코멘트로 → state `Done`.
...
- 리뷰 수정 후 재검증(4번) 생략 금지 — 가장 흔하게 새는 단계
