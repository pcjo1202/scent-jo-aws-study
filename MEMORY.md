...
| 프로덕션 api 상태 | **500 (`FUNCTION_INVOCATION_FAILED`)** — 다만 **원인이 바뀌었다.** `ERR_REQUIRE_ESM`은 해소됐다(SJO-49 / PR #29, `@nestjs/config` 12.0.0→4.0.4). 프리뷰 실측에서 모듈 로딩을 통과해 `ConfigModule.forRoot`까지 들어간 뒤 **`validateEnv`가 `SUPABASE_JWKS_URL`·`SUPABASE_JWT_ISSUER`·`ALLOWED_EMAIL` 누락으로 던진다** — 이제 남은 것은 **SJO-50 하나다**. 프로덕션은 PR #29가 머지되기 전까지 옛 코드라 여전히 ESM 오류다 |
| 프로덕션 web 빌드 | `bcb3164`부터 전부 ERROR였다 — 원인은 **api 장애가 아니라 `/`가 정적 프리렌더였던 것**이다. `force-dynamic`으로 결합을 끊었고 **api가 500인 채로 프리뷰 web이 200을 준다**(2026-09-04 실측, SJO-49 / PR #29). 프로덕션 회복은 머지 시점 |
