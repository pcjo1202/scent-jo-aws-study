...
| 로컬 `tests/fixtures/` | **없음** — 골든 픽스처 6문항이 다른 기기에만 있다. gitignored이고 `data:pull`(SJO-7 미구현)로만 복구된다. `pnpm test`가 이 워크트리에서 1건 실패한다 |
...
- 자동 태깅 실측: 별칭 354개 · 사전 서비스 210개(노트 202 + 루트 보완 8). **미태깅 6문항(0.6%)** — MFA·SAML·STS처럼 노트에 없는 개념만 언급하는 문항이라 사전으로는 닿지 않는다
