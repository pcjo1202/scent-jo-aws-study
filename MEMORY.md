...
| Supabase Auth URL 설정 | **틀려 있다 — 로그인이 web으로 돌아오지 못한다.** 2026-09-06 실제 Google 로그인 실측(SJO-19): `redirectTo=http://localhost:3000/`을 넘겼는데 `https://aws-study-api-smelljo.vercel.app/#access_token=…`으로 떨어져 404(Nest에 `/` 라우트 없음). Supabase는 `redirect_to`가 허용 목록에 없으면 **Site URL로 떨어뜨리므로**, Site URL이 **api 프로젝트**를 가리키고 localhost는 목록에 없다는 뜻이다. 원인은 **Vercel 통합이 api 프로젝트에 연결**돼 있는 것으로 보인다(아래 통합 행). 고칠 곳은 콘솔 **Authentication → URL Configuration** — Site URL을 `https://saa.scent-jo.dev`로, Redirect URLs에 `http://localhost:3000/**`·`https://saa.scent-jo.dev/**`·`https://aws-study-*-smelljo.vercel.app/**`. **`/auth/v1/authorize`는 어떤 `redirect_to`에도 302를 주므로 그 응답으로는 판정할 수 없다** — 검증은 콜백에서 일어난다(2026-09-06 `evil.example`로 음성 대조). MCP에 auth 설정 도구가 없어 코드로는 못 고친다 |
...
| api 인증 환경변수 | **세 키 모두 production·preview·development에 등록됨** (2026-09-06, SJO-50). `env.ts` 필수 4키 × 3환경 = **12칸 중 12칸** (`vercel env ls`를 `REQUIRED_ENV_KEYS`와 대조해 셈). production·preview는 Vercel이 **Sensitive**로 저장해 `vercel env pull`이 값을 가린다 — 값 대조는 development에서만 되고 3/3 문자열 완전 일치. `SUPABASE_JWT_ISSUER`는 `https://<ref>.supabase.co/auth/v1`(**후행 슬래시 없음**) |
...
| api 환경변수 현재값 | production·preview `CORS_ALLOWED_ORIGINS`·`DATABASE_URL`·`SUPABASE_JWKS_URL`·`SUPABASE_JWT_ISSUER`·`ALLOWED_EMAIL` · development `DATABASE_URL`·`SUPABASE_JWKS_URL`·`SUPABASE_JWT_ISSUER`·`ALLOWED_EMAIL` (CORS는 development에 없다 — 로컬은 `.env` 기본값을 쓴다) |
