...
| Supabase Email 프로바이더 | **꺼져 있음** — 2026-09-06 `/auth/v1/settings` 실측 `external.email: false` · `external.anonymous_users: false` · `external.google: true` · `disable_signup: false`. 이전 기록의 「열려 있음」은 낡았다. **로그인 수단이 Google 하나뿐이므로 토큰이 필요한 검증은 사람이 실제로 로그인해야 얻는다** — 임시 계정을 만들 경로가 없다 (SJO-50) |
...
| 프로덕션 api 상태 | **200** — `GET https://aws-study-api.vercel.app/health` → `{"status":"ok","service":"api","version":"d8dc029"}` (2026-09-06 실측, SJO-50). 직전까지 500 `FUNCTION_INVOCATION_FAILED`였고 남은 원인은 인증 환경변수 3개 누락이었다. **등록만으로는 낫지 않아 `vercel redeploy`로 같은 커밋을 다시 태웠다** — 환경변수는 배포 시점에 묶인다 |
