...
| 프로덕션 api 상태 | **200** — `GET https://aws-study-api.vercel.app/health` → `{"status":"ok","service":"api","version":"d8dc029"}` (2026-09-06 실측, SJO-50). **환경변수를 등록해도 기존 배포는 낫지 않는다** — 값은 배포 시점에 묶이므로 `vercel redeploy`로 같은 커밋을 다시 태워야 한다 |
